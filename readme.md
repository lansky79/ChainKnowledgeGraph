...
这是一个基于Streamlit的知识图谱数据导入可视化工具，用于监控和控制Neo4j知识图谱的数据导入进度。

## 功能演示

![导入进度仪表盘](https://user-images.githubusercontent.com/12345678/123456789-abcdef12-3456-7890-abcd-ef1234567890.png)
...
1. 确保已安装Python 3.8+和Neo4j数据库

2. 克隆或下载本仓库

3. 运行启动脚本:
   - Windows: 双击 `start_dashboard.bat`
   - Linux/Mac: 执行 `./start_dashboard.sh`
...
3. 确保Neo4j数据库已经启动并可访问

4. 启动Streamlit应用：

```bash
streamlit run kg_import_dashboard.py
```

## 使用方法

1. 启动应用后，浏览器会自动打开应用页面（默认地址为 http://localhost:8501）

2. 使用界面上的控制面板执行导入操作或查看导入状态

3. 通过不同的标签页查看各种数据可视化和统计信息

## 配置说明

可以通过编辑 `config.json` 文件来自定义应用配置：
...
- 导入大量数据可能需要较长时间，请耐心等待
- 重置导入状态操作会清除所有导入进度记录，请谨慎操作
- 建议定期备份Neo4j数据库
...
3. 查看Neo4j日志以获取更多信息
4. 检查数据文件格式是否正确
5. 如果遇到PowerShell解析错误，尝试使用CMD运行脚本
